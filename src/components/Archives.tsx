  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-full md:w-[95vw] md:h-[90vh] md:max-w-6xl bg-black/20 md:border md:border-white/10 md:rounded-[32px] overflow-hidden flex flex-col shadow-2xl backdrop-blur-md">
        ...
        <div className="flex border-b border-white/5 bg-black/20 overflow-x-auto no-scrollbar p-2 md:p-0">
          <div className="flex gap-2 md:gap-0 w-full">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as Tab)}
                className={`flex items-center gap-2 px-4 md:px-6 py-2 md:py-4 text-[10px] md:text-xs font-mono uppercase tracking-widest transition-all relative min-w-max rounded-full md:rounded-none ${
                  activeTab === tab.id 
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 bg-gradient-to-b from-transparent to-white/[0.02]">
          <AnimatePresence mode="wait">
            {activeTab === 'timeline' && (
              ...
              >
                {(!gameState.chronicles || gameState.chronicles.length === 0) ? (
                  <div className="flex flex-col items-center justify-center h-64 text-white/20">
                    <History size={48} className="mb-4 opacity-20" />
                    <p className="font-mono text-xs uppercase tracking-widest">Летопись пуста</p>
                  </div>
                ) : (
                  [...gameState.chronicles].reverse().map((entry, i) => (
            ...
            {activeTab === 'heroes' && (
              ...
              >
                {(!gameState.heroes || gameState.heroes.length === 0) ? (
                  <div className="col-span-full flex flex-col items-center justify-center h-64 text-white/20">
                    <Users size={48} className="mb-4 opacity-20" />
                    <p className="font-mono text-xs uppercase tracking-widest">Великие личности еще не явились</p>
                  </div>
                ) : (
            ...
            {activeTab === 'museum' && (
              ...
              >
                {(!gameState.relics || gameState.relics.length === 0) ? (
                  <div className="col-span-full flex flex-col items-center justify-center h-64 text-white/20">
                    <Library size={48} className="mb-4 opacity-20" />
                    <p className="font-mono text-xs uppercase tracking-widest">Музей пуст</p>
                  </div>
                ) : (
                  gameState.relics.map((relic) => (
                    <div key={relic.id} className="p-4 md:p-5 bg-white/[0.02] border border-white/10 rounded-2xl flex flex-col items-center text-center hover:bg-white/[0.07] transition-all">
                      ...
                      <p className="text-[10px] md:text-xs text-white/50 leading-relaxed line-clamp-3">{relic.description}</p>
                    </div>
                  ))
                )}
              </motion.div>
            ...
            {activeTab === 'culture' && (
              ...
              >
                <div className="p-6 md:p-8 bg-white/[0.02] border border-white/10 rounded-3xl relative overflow-hidden">
                  ...
                  <h3 className="text-[8px] md:text-xs font-mono uppercase tracking-[0.3em] text-gold mb-4 md:mb-6">Текущая Идеология</h3>
                  <div className="text-2xl md:text-4xl font-light text-white mb-3 md:mb-4 uppercase tracking-widest">{gameState.ideology}</div>
                  <p className="text-white/40 text-xs md:text-sm max-w-2xl leading-relaxed">
                    Этот путь определяет мировоззрение вашего народа, их отношение к богам, природе и друг другу. Идеология формируется под влиянием ваших решений и текущей эпохи.
                  </p>
                </div>

                <div>
                  <h3 className="text-[8px] md:text-xs font-mono uppercase tracking-[0.3em] text-white/40 mb-4 md:mb-6 px-2">Внешние Контакты</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                    {(!gameState.externalContacts || gameState.externalContacts.length === 0) ? (
                      <div className="col-span-full p-6 md:p-8 border border-dashed border-white/10 rounded-2xl text-center text-white/20 font-mono text-[10px] md:text-xs uppercase tracking-widest">
                        Контакты с другими цивилизациями не установлены
                      </div>
                    ) : (
                      gameState.externalContacts.map((contact, i) => (
              ...
              >
                <div className="p-6 md:p-8 bg-white/[0.02] border border-white/10 rounded-3xl">
                  <div className="flex items-center justify-between mb-6 md:mb-8">
                    <h3 className="text-[8px] md:text-xs font-mono uppercase tracking-[0.3em] text-gold">Прогресс Финала</h3>
                    <span className="text-gold font-mono text-lg md:text-xl">{gameState.victoryProgress}%</span>
                  </div>
                  ...
                  <p className="text-white/40 text-[10px] md:text-sm leading-relaxed text-center font-mono uppercase tracking-widest">
                    Ваша цивилизация стремится к высшей цели. Каждый шаг приближает вас к моменту, когда история станет легендой.
                  </p>
                </div>
                ...
                <div className="p-6 md:p-8 bg-white/[0.02] border border-white/10 rounded-3xl flex items-start gap-4 md:gap-6">
                  ...
                  <div>
                    <h3 className="text-[8px] md:text-xs font-mono uppercase tracking-[0.3em] text-white/40 mb-1 md:mb-2">Звуковой Ландшафт Эпохи</h3>
                    <p className="text-base md:text-xl text-white/80 font-light italic leading-relaxed">
                      "{gameState.currentSoundscape}"
                    </p>
                  </div>
                </div>
              </motion.div>
