...
export const generateFateOutcome = async (
  ...
  try {
    ...
    const prompt = `
      ...
      Если количество навыков ПРОГРЕССА (сейчас: ${gameState.skills.filter(s => s.type === 'progress').length}) достигло 25 И прошло минимум 200 лет с начала текущей эпохи (сейчас: ${gameState.eraYear} лет), сгенерируйте НОВУЮ ЭПОХУ.
      - Название эпохи должно быть уникальным, но вдохновленным классическими этапами.
      ...
      - victoryProgressIncrement: Число от 1 до 5, отражающее прогресс к "финалу" цивилизации.
      ...
      ОТВЕТЬТЕ СТРОГО В ФОРМАТЕ JSON:
      {
        ...
        "victoryProgressIncrement": number,
